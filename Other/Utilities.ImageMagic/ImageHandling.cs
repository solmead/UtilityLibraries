...
namespace Utilities.ImageMagic
{
    public class ImageHandling
    {
        ...
        public static string GetResizedImage(string imageFile, ImageSize imageSize)
        {
            return GetResizedImage(imageFile, imageSize.Width, imageSize.Height, imageSize.Stretch, imageSize.ImageType, imageSize.FillArea, imageSize.HAlignment, imageSize.VAlignment, imageSize.JpegQuality);
        }
        public static string GetResizedImage(string imageFile, int width, int height, bool stretch = false, ImageType imageType = ImageType.JPG, bool fillArea = false, HAlignment_Enum hAlignment = HAlignment_Enum.Center, VAlignment_Enum vAlignment = VAlignment_Enum.Center, int jpegQuality = 100)
        {
            var finImage = "";
            var cDir = Directory.GetCurrentDirectory();
            var fi = new FileInfo(Path.Combine(cDir, imageFile));
            if (fi.Exists)
            {
                ...
                var f = fi.FullName.Replace(imageFile.Replace("//", "/").Replace("/", "\\"), "");
